...
import { SocialQuotes } from '../shared/SocialQuotes';
import {
...
class Social extends Component {
    ...
    Next = () => {
        this.setState({ index: (this.state.index + 1) % SocialQuotes.length });
    };

    Prev = () => {
        if (this.state.index === 0) {
            this.state = {
                index: SocialQuotes.length
            }
        }
        this.setState({ index: (this.state.index - 1) % SocialQuotes.length });
    };
    ...
    render() {
        ...
        const Quote = SocialQuotes[this.state.index];
