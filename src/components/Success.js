...
import { SuccessQuotes } from '../shared/SuccessQuotes';
...
class Success extends Component {
    ...
    Next = () => {
        this.setState({ index: (this.state.index + 1) % SuccessQuotes.length });
    };

    Prev = () => {
        if (this.state.index === 0) {
            this.state = {
                index: SuccessQuotes.length
            }
        }
        this.setState({ index: (this.state.index - 1) % SuccessQuotes.length });
    };
    ...
    render() {
        ...
        const Quote = SuccessQuotes[this.state.index];
